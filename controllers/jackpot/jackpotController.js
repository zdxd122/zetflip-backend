...
exports.get_jackpot = asyncHandler(async (req, res, next) => {
  ...
  // If no jackpot exists, return empty data
  if (!activeJackpot) {
...
async function startupCheckUnfinished() {
  ...
  if (!currentJackpot) {
    // No jackpot exists, create a new one
    create_jackpot();
    return;
  }
  ...
  } else if (currentJackpot.state == "Ended" && currentJackpot.inactive == false) {
    play_jackpot();
    setTimeout(async () => {
      await Jackpot.findByIdAndUpdate(currentJackpot._id, {
        inactive: true,
  ...
  } else if (currentJackpot.state == "Ended" && currentJackpot.inactive == true) {
    create_jackpot();
  }
}
